...
install_xui() {
    ...
    # Install awg-helper.sh to /usr/local/bin/
    if [ -f "awg-helper.sh" ]; then
        cp awg-helper.sh /usr/local/bin/awg-helper.sh
        chmod +x /usr/local/bin/awg-helper.sh
        echo -e "${green}Installed awg-helper.sh to /usr/local/bin/${plain}"
    fi
