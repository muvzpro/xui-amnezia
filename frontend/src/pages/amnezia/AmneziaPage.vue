...
async function fetchServers() {
  loading.value = true;
  try {
    const msg = await HttpUtil.get('/panel/api/amnezia/servers');
    if (msg?.success) {
      servers.value = msg.obj || [];
    }
  } finally {
    loading.value = false;
    fetched.value = true;
...
async function fetchPeers(serverId) {
  loading.value = true;
  try {
    const msg = await HttpUtil.get(`/panel/api/amnezia/servers/${serverId}/peers`);
    if (msg?.success) {
      peers.value = msg.obj || [];
    }
  } finally {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      if (msg?.success) {
        ...
        await fetchPeers(peer.serverId);
      }
    },
  });
}
...
async function onSavePeer(payload) {
  ...
  if (msg?.success) {
    ...
    await fetchPeers(peerFormServerId.value);
    return true;
  }
  return false;
}
...
async function onExtendPeerConfirm(days) {
  ...
  if (msg?.success) {
    ...
    const serverId = peers.value.find(p => p.id === extendPeerId.value)?.serverId;
    if (serverId) await fetchPeers(serverId);
    return true;
  }
  return false;
}
...
async function onDownloadConfig(peer) {
  const msg = await HttpUtil.get(`/panel/api/amnezia/peers/${peer.id}/config`);
  if (msg?.success || msg?.obj) {
    const config = msg.obj || msg;
            ...
            <a-row v-else :gutter="[isMobile ? 8 : 16, isMobile ? 0 : 12]">
              ...
              <a-col :span="24">
                <a-card :title="t('amnezia.servers')" hoverable>
                  ...
                  <a-table :data-source="servers" :row-key="record => record.id" :pagination="{ pageSize: 10 }">
                    ...
                    <a-table-column :title="t('amnezia.status')" data-index="enabled">
                      <template #default="{ record }">
                        <a-tag :color="record.enabled ? 'green' : 'red'">
                          {{ record.enabled ? t('amnezia.statusActive') : t('amnezia.statusPaused') }}
                        </a-tag>
                      </template>
                    </a-table-column>
                      ...
                      <template #default="{ record }">
                        <a-space>
                          <a-tooltip :title="t('amnezia.editServer')">
                            <a-button size="small" @click="onEditServer(record)">
                              <EditOutlined />
                            </a-button>
                          </a-tooltip>
                          <a-tooltip :title="t('amnezia.startServer')">
                            <a-button size="small" @click="onStartServer(record)" :disabled="record.enabled">
                              <PlayCircleOutlined />
                            </a-button>
                          </a-tooltip>
                          <a-tooltip :title="t('amnezia.stopServer')">
                            <a-button size="small" @click="onStopServer(record)" :disabled="!record.enabled">
                              <PauseCircleOutlined />
                            </a-button>
                          </a-tooltip>
                          <a-tooltip :title="t('amnezia.restartServer')">
                            <a-button size="small" @click="onRestartServer(record)">
                              <ReloadOutlined />
                            </a-button>
                          </a-tooltip>
              ...
              <a-col :span="24">
                <a-card :title="t('amnezia.peers')" hoverable>
                  <template #extra>
                    <a-button type="primary" @click="onAddPeer(servers[0]?.id)" :disabled="!servers.length">
                      <PlusOutlined />
                      {{ t('amnezia.addPeer') }}
                    </a-button>
                  </template>
